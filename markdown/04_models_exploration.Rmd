...
parameters <- GetTrainTestDatasetsParameters(target_data$date, 0.7)
...
plot(train_ts, 
     ...
     ylab = "Vendas", 
     xaxt = "n",
     xlim = c(parameters$start, parameters$end),
     ylim = c(0, 10), 
     bty = "l")

axis(1, at = seq(parameters$start, parameters$end, 1), 
     labels = format(seq(parameters$start, parameters$end, 1)))
...
O modelo de suavização exponencial **`r mape_ranking[1,1]`** demonstrou a melhor métrica de **MAPE**, com o valor de **`r min(mape_ranking[,2])`**.
