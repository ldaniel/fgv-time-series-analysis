...
parameters <- GetTimeSeriesDateParameters(target_data$date, 0.8)
print(parameters)
```
...
O modelo **`r mape_ranking[1,2]`** demonstrou a melhor métrica de **MAPE**, com o valor de **`r min(mape_ranking[,3])`**.
...
### Checando a aderencia do modelo na base de validação
...
title(sub = 'Verificação do Modelo na base de validação', col.sub = 'blue',
      xlab = "Tempo",
      ylab = "Indice")
