...
axis(1, at = seq(2005, 2019, 1), labels = format(seq(2005, 2019, 1)))
...
naive_consolidation <- GenerateNaiveTimeSeriesModel(train_ts, test_ts)
```
...
ma_consolidation <- GenerateMovingAverageTimeSeriesModel(target_ts, train_ts, test_ts)
```
...
O modelo de suavização exponencial **`r min(mape_ranking[,1])`** demonstrou a melhor métrica de **MAPE**, com o valor de **`r min(mape_ranking[,2])`**.
...
plot(best_model$model_final_projected, 
     ...
     xlim = c(2005, 2019),
     ylim = c(0, 10), 
     flty=2, main="Forecast from Polynomial regression model")
axis(1, at=seq(2005, 2019, 1), labels=format(seq(2005, 2019, 1)))
